...
| `target_resolution` | **🆕 v7** 目标分辨率**白名单**, 多个用 `;` 分隔。默认 `1920x1088;1088x1920;544x960;960x544`。留空 / `*` / `auto` = 不过滤(任意分辨率都收, 取最新视频) |
| `register_only_target_res` | 登记视频时是否按 `target_resolution` 过滤。关掉 = 直接取最新视频不管分辨率 |
...
- **📐 编辑目标分辨率(白名单)** — **🆕 v7** 弹窗 chip 增删,9 个常用分辨率预设(1920×1088 / 1088×1920 / 544×960 / 960×544 / 1280×720 / 720×1280 / 1024×1024 / 832×480 / 480×832)。清空 = 不过滤分辨率(任意视频都收)
- **📖 提示词手册** — **🆕 v7** 保存常用 prompt,左侧列表 + 右侧编辑表单,支持新增/更新/删除/搜索/标签/一键复制。数据持久化到 `ComfyUI/user/auto_sequential_prompts.json`,全局共享(不绑节点 ID)
...
### 为啥要 `target_resolution`(v7 起改为白名单)

有些工作流一次会输出**两个视频**：一个低分辨率预览版（如 720p）+ 一个 1920×1088 的最终版。
如果不过滤分辨率，可能会抽到预览版的最后一帧 → 输入 LoadImage 时分辨率对不上 → 报错或生成出来变形。

**v7 起 `target_resolution` 改为「白名单」**, 默认填 `1920x1088;1088x1920;544x960;960x544`,
覆盖横屏 / 竖屏 / 横竖低分辨率四种常见情形。插件只会从命中**任一**分辨率的视频里抽末帧。

写法都兼容: `1920x1088` / `1920*1088` / `1920×1088`, 分隔符 `;` `,` 空格 换行 都行。
- **多个分辨率**: `1920x1088;1088x1920;544x960;960x544`
- **任意分辨率(不过滤)**: 留空, 或填 `*` / `any` / `auto`,这种情况下直接取最新视频
- **只要单一分辨率**: `1920x1088`

不想改字符串的可以点节点上的 **📐 编辑目标分辨率(白名单)** 按钮,弹窗 chip 增删,
还有 9 个常用预设一键添加。

### 「不管什么分辨率自动取最后一段视频」怎么做

两种途径,看场景挑:

1. **临时一次性绕过**: 在节点上把 `register_only_target_res` 关掉。这样登记视频时**完全忽略**
   分辨率,取 `since_timestamp` 之后最新的那一个。`target_resolution` 字段保留不变,后续想开过滤
   再勾回来。
2. **彻底关掉过滤**: 点 **📐 编辑目标分辨率(白名单)** → 「🗑 清空 (= 任意分辨率)」→ 确定。
   `target_resolution` 字段变空,等同于不过滤。

### 怎么用

1. 像之前一样配好 directory / 选好 LoadImage（参考上面的标准用法）
2. 把 **tail_frame_mode** 勾上 ✅
3. 确认 **target_resolution** 包含你工作流输出的分辨率(默认 `1920x1088;1088x1920;544x960;960x544` 已覆盖横/竖屏常见情形)
...
为了同时跑横屏 / 竖屏 / 不同低分辨率工作流, `target_resolution` 从单一字符串升级成**白名单**, 多个分辨率用 `;` 分隔:

```
1920x1088;1088x1920;544x960;960x544
```

匹配规则: 视频分辨率命中**任一**就算过, 命中不上就跳过 (历史行为)。

**怎么改这个白名单**:

1. **直接在 widget 里编辑**字符串 (高级用户)
2. **点节点上的 📐 编辑目标分辨率(白名单) 按钮** (推荐) —— 弹窗 chip 增删, 内置 9 个常用预设:
...
**特殊值**:
- 留空 / `*` / `any` / `auto` = 任意分辨率(不过滤, 取最新视频)
- 想保留白名单但临时不过滤: 把 `register_only_target_res` 关掉

---

## 🆕 提示词手册 (v7+)

ComfyUI 写视频 prompt 容易写到长长的一串, 又老想复用以前调好的版本。**📖 提示词手册** 按钮就是给这个用的小本本:

- 点 ➕ 新建, 填**标题** + **内容** + **标签** (逗号或空格分隔), 💾 保存
- 左侧列表点条目 → 右侧编辑表单, 改完 💾 保存就是更新
- **📋 复制内容** 一键复制到剪贴板, 粘贴到 prompt 节点即可
- 🔍 搜索框: 按标题 / 内容 / 标签实时筛选
- 🗑 删除单条; 没选中条目时点删除 = 询问是否清空全部
- 数据持久化在 `ComfyUI/user/auto_sequential_prompts.json`, 全局共享 (不绑节点 ID), 重启 ComfyUI 还在

后端接口 (REST):

| 路径 | 方法 | body |
|---|---|---|
| `/auto_sequential/prompts/list` | POST | `{search?, tag?}` |
| `/auto_sequential/prompts/save` | POST | `{id?, title, content, tags?}` (id 为空 = 新增) |
| `/auto_sequential/prompts/delete` | POST | `{id}` 或 `{all: true}` |
